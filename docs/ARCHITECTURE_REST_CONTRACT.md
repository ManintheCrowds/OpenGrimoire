...
| Operator lifecycle + DB diagnostics | yes (`/api/admin/db/health`, `/api/admin/db/lifecycle`) | yes (`/api/admin/db/lifecycle`) | — | — | Operator session cookie | `/api/admin/db/health` reports SQLite journal/schema diagnostics; `/api/admin/db/lifecycle` supports retention policy read, prune/backup actions, and table export. |
...
| Local AI activity adapter (cockpit) | yes (`/api/admin/cockpit/local-ai/activity`) | yes (`/api/admin/cockpit/local-ai/activity`) | — | — | Operator session cookie | GET reads local JSONL adapter, POST appends local activity events (operator-only telemetry path). |
| UX metrics adapter (cockpit) | yes (`/api/admin/cockpit/ux-metrics`) | — | — | — | Operator session cookie | Derived operator UX metrics from local activity events (accept/dismiss/undo + daily trend). Adapter-grade, not canonical BI pipeline. |
