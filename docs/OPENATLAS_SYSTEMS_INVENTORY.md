# OpenAtlas — systems inventory (tools, APIs, workflows)

**Purpose:** Single place to list what OpenAtlas touches and what agents use around it. **Workspace-wide MCP and browser tooling** stay canonical in [`.cursor/docs/MCP_CAPABILITY_MAP.md`](../../.cursor/docs/MCP_CAPABILITY_MAP.md) — do not duplicate that full matrix here; link it instead.
...
| **OpenAtlas** (this app under `portfolio-harness/OpenAtlas`) | **Implementation**: Next.js UI + APIs + visualization. Consumes **brain-map JSON** produced by harness-adjacent scripts. |

**OpenAtlas is not a submodule of OpenHarness.** Nesting this Next.js app inside the public harness repo would violate [OpenHarness `docs/DELINEATION.md`](../../../openharness/docs/DELINEATION.md) (domain-specific, heavy deps). The intended split: **harness = patterns + state**; **OpenAtlas = optional viewer + portfolio-specific features**.
...
|------|-------------|--------|
| Route | `/`, `/visualization`, `/login`, `/admin/*` | Supabase areas need env |
...
| `node scripts/alignment-context-cli.mjs` | Alignment API CLI (`OPENATLAS_BASE_URL`, etc.) |
