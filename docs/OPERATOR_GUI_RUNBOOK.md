# Operator runbook — OpenGrimoire (OpenAtlas) GUI
...
| Alignment (admin) | Requires Supabase + admin role: `/login` then `/admin/alignment`. Focus/visibility triggers refetch for external CLI changes. |
...
| Admin redirects to login | Supabase env not configured or user not admin. |

---

## OpenHarness (no browser)

OpenAtlas is the **browser app**. **OpenHarness** is a separate repo: portable **markdown workflows**, `.cursor` rules/skills, and `state/`—not a React GUI. When your work is handoffs, scripts, or skills rather than the web UI, use the harness docs and paths below.
...
| Handoff integrity + portfolio scripts (monitoring split) | [MONITORING_OPENATLAS.md](./MONITORING_OPENATLAS.md) (OpenHarness section) |
...
**Relationship:** [OPENATLAS_SYSTEMS_INVENTORY.md](./OPENATLAS_SYSTEMS_INVENTORY.md) explains how OpenHarness patterns and brain-map builds relate to this app.
...
- [MONITORING_OPENATLAS.md](./MONITORING_OPENATLAS.md) — where logs/metrics live.
