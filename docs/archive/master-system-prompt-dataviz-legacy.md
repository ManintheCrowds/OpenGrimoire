...
```markdown
# OpenGrimoire — Data visualization implementation guide

## Context
You are implementing data visualizations for the OpenGrimoire (Agent Context Atlas) survey app. The core message to communicate is "We are different, but we are also similar" — showing how respondents connect across various dimensions.
...
2. The data includes responses about:
   - Tenure (years), field `tenure_years`
   - Learning styles
