...
**Static bypass:** Files may still exist under `public/` for the server route to read, but **direct** requests to `/brain-map-graph.json`, `/brain-map-graph.local.json`, and renamed backups of those files (e.g. `.bak`, `.pre_e2e_backup`) are **blocked** (404) so clients must use `GET /api/brain-map/graph` with your chosen auth posture. Do not leave vault-inclusive graph copies under `public/` with alternate extensions — middleware blocks known name patterns, but the files still ship with the deployment artifact.
