...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (error) {
    if (process.env.NODE_ENV === 'development') {
      console.error('[admin/clarification-requests] GET:', error.code, error.message);
    } else {
      console.error('[admin/clarification-requests] GET:', error.code);
    }
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (result.error) {
    if (process.env.NODE_ENV === 'development') {
      console.error('[admin/clarification-requests] PATCH:', result.error.code, result.error.message);
    } else {
      console.error('[admin/clarification-requests] PATCH:', result.error.code);
    }
