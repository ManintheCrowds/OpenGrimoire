...
/**
 * GET /api/clarification-requests — list (agent poll). POST — create pending item.
 * Auth: same shared-secret gate as alignment context (`x-alignment-context-key`).
 */
export async function GET(request: Request) {
  ...
  if (error) {
    if (process.env.NODE_ENV === 'development') {
      console.error('[clarification-requests] list:', error.code, error.message);
    } else {
      console.error('[clarification-requests] list:', error.code);
    }
...
export async function POST(request: Request) {
  ...
  if (error) {
    if (process.env.NODE_ENV === 'development') {
      console.error('[clarification-requests] insert:', error.code, error.message);
    } else {
      console.error('[clarification-requests] insert:', error.code);
    }
